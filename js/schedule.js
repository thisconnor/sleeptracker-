...
export function energyAt(tHours, tBedHours, energyScore = 100) {
  ...
  const e =
    ...
    dip.amp * gauss(tHours, dip.mu, dip.sigma) +
...
export function buildZones(wake, bed) {
  ...
  const zones = [
    ...
    { id: 'dip', label: 'Afternoon dip', start: W + 7 * H, end: W + 9.5 * H },
...
export function curvePoints(wake, needMin, energyScore, stepMin = 10) {
  ...
  for (let m = 0; m <= 1440; m += stepMin) {
    ...
    points.push({ time, value: energyAt(tHours, tBedHours, energyScore) });
  }
  return points;
}

// Convenience: everything the schedule card needs.
export function buildSchedule(nights, needMin, energyScore, now = new Date()) {
  ...
  return {
    ...
    zones: buildZones(wake, bed),
    points: curvePoints(wake, needMin, energyScore),
  };
}
